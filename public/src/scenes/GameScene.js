...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    if(this.mode==='boss' || this.mode==='hardboss') {
      ...
      // Throw timer
      const speed = this.mode==='boss'?300:150;
      this.time.addEvent({
  ...
  update() {
    ...
    if(this.boss){
      ...
      // else stays where it dived
    }

    // Spears: despawn only in Boss, homing in both
    if(this.spears){
      const cam = this.cameras.main;
      this.spears.getChildren().forEach(sp=>{
        if(this.mode==='boss' && (
            sp.y > cam.scrollY + cam.height + 50 ||
            sp.x < cam.scrollX - 50 ||
    ...
    // Fall teleport 30px above center
