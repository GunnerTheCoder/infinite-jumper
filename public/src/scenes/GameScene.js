...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    // UI
    this.add.text(780,10,`Seed: ${this.seed}`,{font:'20px Arial',fill:'#fff',stroke:'#000',strokeThickness:3})
    ...
    if(this.mode==='boss'){
      ...
      this.boss = this.add.image(0,0,'boss').setScrollFactor(0);
      this.time.addEvent({
        delay:1500, loop:true, callback:()=>{
          const cam = this.cameras.main;
          const bx = Phaser.Math.Clamp(this.player.x, cam.scrollX+64, cam.scrollX+this.scale.width-64);
  ...
  update() {
    ...
    // Chaser AI (follows your X exactly)
    if(this.chaser){
      this.chaser.setVelocityX((this.player.x > this.chaser.x ? 1 : -1) * SPEED*0.8);
    ...
    if(this.lastX < camR){
      const newY = Phaser.Math.Clamp(this.lastY + Phaser.Math.Between(-100,100), 300,550);
      this.spawnPlatform(this.lastX + Phaser.Math.Between(100,220), newY);
    }

    // Void check & teleport
    const voidY = this.cameras.main.scrollY + 700;
    ['player','chaser'].forEach(key=>{
      const obj = this[key];
      if(obj && obj.y > voidY) this.teleportAboveNearest(obj);
    });
  }
  ...
  teleportAboveNearest(sprite){
    ...
    if(best){
      sprite.setPosition(best.x + best.displayWidth/2,
                         best.y - sprite.displayHeight/2);
